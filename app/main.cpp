	...
	const long max_itr = 1e6)
{
	...
	int max_fnbr = is_simulation_data ? simulation_folders.size() : handcrafted_folders.size();
	const auto folders
			...
			for (int i = 0; i < v_size; i++) {
				...
				if (is_edge_edge) {
					new_result = edgeEdgeCCD_double(
						...
						V.row(5), V.row(6), V.row(7), err,minimum_seperation,toi,tolerance, 
...
void run_code() {
	...
	run_rational_data_single_method(/*is_edge_edge*/false,/*is_simulation*/ false, ms, tolerance);
	std::cout << "finish Vertex-Face data" << std::endl;
	std::cout << "\nRunning Edge-Edge data" << std::endl;
	run_rational_data_single_method(/*is_edge_edge*/true,/*is_simulation*/ false, ms, tolerance);
